...
class Network:
    ...
    def run(self, *in_arrays,
        ...
        **dynamic_kwargs):          # Additional keyword arguments to pass into the network construction function.
        ...
        for mb_begin in range(0, num_items, minibatch_size):
            if print_progress:
                print('\r%d / %d' % (mb_begin, num_items), end='',flush=True)
        ...
        # Done.
        if print_progress:
            print('\r%d / %d' % (num_items, num_items),flush=True)
    ...
    def print_layers(self, title=None, hide_layers_with_no_params=False):
        ...
        print("----------",flush=True)
        print('%-28s%-12s%-24s%-24s' % (title, 'Params', 'OutputShape', 'WeightShape'),flush=True)
        print('%-28s%-12s%-24s%-24s' % (('---',) * 4),flush=True)
        ...
        for layer_name, layer_output, layer_trainables in self.list_layers():
            ...
            print('%-28s%-12s%-24s%-24s' % (
                ...
                layer_output.shape,
                weights[0].shape if len(weights) == 1 else '-'),flush=True)

        print('%-28s%-12s%-24s%-24s' % (('---',) * 4),flush=True)
        print('%-28s%-12s%-24s%-24s' % ('Total', total_params, '', ''),flush=True)
        print("----------",flush=True)
