...
class TFRecordDataset:
    def __init__(self,
        ...
        # Remove data files above resolution if resolution is defined 
        if resolution is not None:
            cut_off = np.int(np.log2(resolution))
            tfr_files = [file for file in tfr_files if np.int(file[-12:-10]) <= cut_off]
        ...
        print("----------------------------", flush=True)
        print("Max resolution used:", self.resolution, flush=True)
        print("----------------------------", flush=True)
...
def load_dataset(class_name='dataset.TFRecordDataset', data_dir=None, verbose=False, **kwargs):
    ...
    if 'tfrecord_dir' in adjusted_kwargs and data_dir is not None:
        adjusted_kwargs['tfrecord_dir'] = data_dir + "/train"
        print("Load Data from:", adjusted_kwargs['tfrecord_dir'], flush=True)
    if verbose:
        print('Streaming data using %s...' % class_name, flush=True)
