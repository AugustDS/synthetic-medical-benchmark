...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    wgan_target     = 1.0,
    cond_weight     = 1.0):     # Target value for gradient magnitudes.
    ...
    if D.output_shapes[1][1] > 0:
        with tf.name_scope('LabelPenalty'):
            ...
            label_penalty_fakes = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=fake_labels_out)
            label_penalty_reals = tfutil.autosummary('Loss/label_penalty_reals', label_penalty_reals*cond_weight)
            label_penalty_fakes = tfutil.autosummary('Loss/label_penalty_fakes', label_penalty_fakes*cond_weight)
